...
export async function handleTwitterInfo(url, res) {
  try {
    console.log("Fetching Twitter video info for:", url);
...
export async function handleTwitterDownload(url) {
  const folderName = crypto.randomUUID();
  const folderPath = `Downloads/${folderName}`;
  fs.mkdirSync(folderPath);

  console.log("Downloading Twitter video from:", url);
  ...
  console.log("Twitter download complete:", filePath);
